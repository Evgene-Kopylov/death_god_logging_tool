...
- **Гибкое управление выводом** - разделение консольного вывода и записи в файл
...
### Параметр `console_output`

Новый параметр `console_output` позволяет гибко управлять выводом:

```rust
// Только консольный вывод (файл не создается)
conflog::init("trace".to_string(), None, true).ok();

// Только запись в файл (без консольного вывода)
conflog::init("trace".to_string(), Some("logs".to_string()), false).ok();

// И консольный вывод, и запись в файл
conflog::init("trace".to_string(), Some("logs".to_string()), true).ok();

// Автоматическое определение режима (debug = консоль, release = файл)
...
- `<app_name>_rCURRENT.log` - текущий файл логов
- `<app_name>_r00000.log` - предыдущий запуск
- `<app_name>_r00001.log` - запуск до предыдущего
- и т.д. (всего хранится 5 файлов)

**Примечание:** Имя приложения определяется автоматически из `std::env::current_exe()`
...
- ✅ Работу параметра `console_output`


## 🔧 API

### `conflog::init(log_level: String, log_path: Option<String>, console_output: bool) -> Result<(), Error>`

Инициализирует систему логирования.

**Параметры:**
- `log_level` - уровень логирования (например, "trace", "debug", "info", "warn", "error")
- `log_path` - опциональный путь к директории для логов
- `console_output` - флаг отображения логов в консоли
